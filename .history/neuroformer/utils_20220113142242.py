...
@torch.no_grad()
def predict_raster_recursive(model, loader, stoi, get_dt=False, sample=False, top_k=None, frame_end=0, id_prev=0, gpu=False):
    device = 'cpu' if not gpu else torch.cuda.current_device() # torch.cuda.current_device() if torch.cuda.is_available() else "cpu"
    model = model.to(device)
    model.eval()
    tf = frame_end
...
def predict_raster_recursive_time(model, loader, stoi, itos_dt, get_dt=False, sample=False, top_k=None, frame_end=0):
    ...
    for it, (x, y) in pbar:
        ...
        if get_dt:
            ...
            true_timing = torch.cat((true_timing.to('cpu'), ix_time_y))     # y['dt'][:, t - pad].flatten())) 
