...
class SpikeTimeVidData2(Dataset):
        ...
        def __getitem__(self, idx):
                ...
                id, x['dt'], x['pad'] = self.get_interval(current_int, t['Trial'])
                x['id'] = torch.tensor(x['id'][:-1], dtype=torch.long)
                x['dt'] = torch.tensor(x['dt'][:-1], dtype=torch.float) + 1
                x['pad'] = torch.tensor(x['pad'], dtype=torch.long)
