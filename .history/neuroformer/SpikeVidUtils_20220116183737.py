...
class SpikeTimeVidData2(Dataset):
        ...
        def get_interval(self, interval, trial):
                ...
                dt_chunk = (data['Time'] - data['Time'].min())
                dt_chunk = ([1] + dt_chunk.tolist() + [1 + 0.2] * (pad_n + 1))[-self.id_block_size:] # 0 = SOS, max = EOS
            
                return dix[-self.id_block_size:], dt_chunk[-self.id_block_size:], pad_n
        ...
        def __getitem__(self, idx):
                ...
                id_prev, dt_prev, _ = self.get_interval(prev_int, t['Trial'])
                ...
                idn, dt, pad = self.get_interval(current_int, t['Trial'])
                x['id'] = torch.tensor(idn[:-1], dtype=torch.long)
                x['dt'] = torch.tensor(dt[:-1], dtype=torch.float) + 1
                x['pad'] = torch.tensor(pad, dtype=torch.long) + 1 # to attend eos
                ...
                y['dt'] = torch.tensor(dt[1:], dtype=torch.float)
