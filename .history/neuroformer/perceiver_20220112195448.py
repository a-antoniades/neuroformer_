...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        ## GPT
        self.stimulus_decoder = BlockSequential(*[Block(config) for _ in range(config.n_layer)])
        self.history_blocks = BlockSequential(*[Block(config) for _ in range(config.n_layer)])
        self.ln_f = nn.LayerNorm(config.n_embd)
        ## enc_dec
