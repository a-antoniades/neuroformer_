    ...
    def __init__(self, config):
        ...
        if config.class_weights is not None:
            for key in config.class_weights.keys():
                self.register_buffer(d"class_weights", config.class_weights[key])  
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            ...
            for B, P in enumerate(pad):
                ...
                loss_id_ = F.cross_entropy(id_logits.view(-1, id_logits.size(-1)), id_targets.view(-1), weight=self.class_weights['id'])
                ...
                loss_time_ = F.cross_entropy(time_preds.view(-1, time_preds.size(-1)), time_targets.view(-1), weight=self.class_weights['dt'])
