...
@torch.no_grad()
def predict_raster_recursive_time(model, loader, stoi, itos_dt, get_dt=False, sample=False, top_k=0, top_p=0, frame_end=0):
    """
    ...
    for it, (x, y) in pbar:
        ...
        for i in range(t - pad):
            ...
            # optionally crop probabilities to only the top k options
            if top_k or top_p is not 0:
                logits['id'] = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
                logits['dt'] = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
