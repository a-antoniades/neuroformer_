...
class SpikeTimeVidData2(Dataset):
        ...
        def __getitem__(self, idx):
                ...
                id_prev'], x['dt_prev'], pad_n = self.get_interval(prev_int, t['Trial'])
                x['id_prev'] = torch.tensor(x['id_prev'][:-1], dtype=torch.long)
                x['dt_prev'] = torch.tensor(x['dt_prev'][:-1], dtype=torch.float) + 0.5
