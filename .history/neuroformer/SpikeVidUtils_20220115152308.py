...
class SpikeTimeVidData2(Dataset):
        ...
        def __getitem__(self, idx):
                ...
                id, dt, pad = self.get_interval(current_int, t['Trial'])
                x['id'] = torch.tensor(id[:-1], dtype=torch.long)
                x['dt'] = torch.tensor(dt[:-1], dtype=torch.float) + 1
                x['pad'] = torch.tensor(pad, dtype=torch.long)
                ...
                y['id'] = torch.tensor(id[1:], dtype=torch.long)
                y['dt'] = torch.tensor(dt[1:], dtype=torch.float)
                
                if self.pred:
                    dt_real = np.array(dt[1:]) + data_current['Time'].min()
