...
data_dir = "./data/VisNav_VR_Expt/MedialVRDataset"
# data_dir = "./data/VisNav_VR_Expt/LatervalVRDataset"

if not os.path.exists(data_dir):
    print("Downloading data...")
    import gdown
    url = DataLinks.LatervalVRDataset
    gdown.download_folder(id=url, quiet=False, use_cookies=False, output=data_dir)
...
base_path = "configs/visnav/predict_behavior/multiple_contrastive"
...
data_path = os.path.join(data_dir, "experiment_data.mat")
...
# %%
# data2path = "data/VisNav_VR_Expt/yiyi/experiment_data.mat"
# data = mat73.loadmat(data2path)['neuroformer']
...
model_path = f"""./models/tensorboard/visnav/behavior_pred_exp/multiple_contrastive/{title}/sparse_f:{mconf.sparse_topk_frame}_id:{mconf.sparse_topk_id}/w:{window}_wp:{window_prev}/{6}_Cont:{mconf.contrastive}_window:{window}_f_window:{frame_window}_df:{dt}_blocksize:{id_block_size}_conv_{conv_layer}_shuffle:{shuffle}_batch:{batch_size}_sparse_({mconf.sparse_topk_frame}_{mconf.sparse_topk_id})_blocksz{block_size}_pos_emb:{mconf.pos_emb}_temp_emb:{mconf.temp_emb}_drop:{mconf.id_drop}_dt:{shuffle}_2.0_{max(stoi_dt.values())}_max{dt}_{layers}_{mconf.n_head}_{mconf.n_embd}.pt"""
...
# %%
# init a loader with batch 1 and shuffle false
loader = DataLoader(train_dataset, batch_size=2, shuffle=False, num_workers=4, pin_memory=True)
iterable = iter(loader)

# %%
# print all keys and shapes of the x, y dicts
x, y = next(iterable)
for key in x.keys():
